...
class OutputWindow(QWidget):
    def __init__(self, player_id, queue, socket):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle(f'Player {self.player_id} - Output Window')
    ...
    def send_message(self, message):
        ...
        self.socket.sendall(pickle.dumps({"type": "client_message", "content": message, "player_id": self.player_id}))
...
def envoiIndice(joueur, classe, indice, output_window):
    ...
    envoi_socket(paquet_indice, output_window)
...
def envoiCarte(carte, output_window):
    ...
    envoi_socket(paquet_carte, output_window)
...
def envoi_socket(paquet, output_window):
    socket.sendall(pickle.dumps(paquet))
    output_window.update_text("Message envoyé avec succès!")
...
if __name__ == '__main__':
    ...
    output_windows = []  # List to store instances of OutputWindow for each player

    HOST = "localhost"
    PORT = 6666

    for _ in range(2):  # Assuming you have a variable 'number_of_players'
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            output_window = OutputWindow(output_queue, s)
            ...
            output_window.update_text(f"Vous êtes le joueur n°{num_joueur}")
        ...
        while True:
            ...
            if infos_tour["turn"] == num_joueur:
                ...
                envoi_socket("Fin du tour", output_window)
                ...
                if paquet["type"] == "indice":
                    ...
                    if int(paquet["joueur"]) == num_joueur:
                        output_window.update_text(
                            f"{infos_tour['turn']} vous envoie un indice sur {article} {paquet['classe']} {indice} !")
                    else:
                        output_window.update_text(
                            f"{infos_tour['turn']} envoie un indice à {paquet['joueur']} sur {article} {paquet['classe']} {indice} !")
